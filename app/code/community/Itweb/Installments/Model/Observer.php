...
class Itweb_Installments_Model_Observer
{
    ...
    public function salesOrderSaveAfter($observer)
    {
        ...
        if ($Order->getUseInstallments() > 0 && !is_null($_status) && !is_null($_state)) {
            ...
            if ($Agreement->getId() <= 0) {
                ...
                if (null !== $Agreement) {
                    $Order->setState(Itweb_Installments_Helper_Data::ORDER_STATE_PENDING_INSTALLMENT_PAID, Itweb_Installments_Helper_Data::ORDER_STATUS_PENDING_INSTALLMENT_PAID, 'Created Installment Agreement.');
        ...
        if ($Invoice->wasPayCalled()) {
            ...
            if ($Order->getUseInstallments() > 0) {
                ...
                if ($Agreement->getState() == Itweb_Installments_Model_Installments::STATUS_CLOSED) {
                    $Comment = $Order->addStatusHistoryComment('Installment Agreement Fully Paid', Itweb_Installments_Helper_Data::ORDER_STATUS_PAID);
                    $Comment->save();
                }
                unset($Agreement);
            }
    ...
    public function applyInstallmentPayment(Itweb_Installments_Model_Installments $Agreement, Mage_Sales_Model_Order_Invoice $Invoice, $Breakdown)
    {
        ...
        foreach ($Invoice->getAllItems() as $Item) {
            ...
            if (isset($Breakdown[$Item->getOrderItem()->getId()])) {
                $ItemsArr[$Item->getId()] = $Breakdown[$Item->getOrderItem()->getId()]['count_of_payment'] - 1;
            } else {
        ...
        foreach ($ItemsArr as $ItemId => $NumberOfPayments) {
            ...
            if ($Payment->isPaid() === false) {
                ...
                $this->payOnPayment($Agreement, $Payment, $ItemBreakdown['payments'][$NumberOfPayments]);
                if ($Payment->isPaid() === true) {
                    $Comment = $Agreement->getOrder()->addStatusHistoryComment('Installment Payment #' . ($NumberOfPayments + 1) . ' Completed.');
                    $Comment->save();
                }
            }
            if ($HadPayment === true) {
